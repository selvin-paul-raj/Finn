...
Finn consists of a single backend service running FastAPI, communicating with a Neon serverless Postgres database and the Google Gemini API. It is designed to work on **free-tier infrastructure** with mechanisms to handle the limitations of free hosting (such as Render's web service cold-start).
...
| `GEMINI_API_KEY` | Gemini LLM API Key (used for parsing raw messages) | Obtained from [Google AI Studio](https://aistudio.google.com/) |
